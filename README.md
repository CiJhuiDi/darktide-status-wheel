...
- **原版 7 项**：要弹药 / 注意 / 帝皇 / 敌人 / 治疗 / 位置 / 谢谢，均带角色语音 + 聊天 + 场景标记动作
- **FTE 新增 3 项**（整合自 For The Emperor，该 mod 已停更）：帮助 / 是 / 否
- **报告高压**：一键向队友喊「高压！！！」
- **状态 3 项**：大招状态（充能层数 / 冷却进度，可配置格式）、手雷数量、子弹余量（弹夹 + 备弹）

> 轮盘选项按类分组，Mod Options 里每类有**总开关**，每项有**独立开关**：嫌选项多就关掉对应类/项，**轮盘槽位、均分位置、图标大小自动按实际显示数调整**（快捷键同步失效）。
...
- 游戏中按 **V** 打开通信轮盘，悬停到条目松开即触发（默认 14 槽：原版 7 + FTE 新增 3 + 报告高压 + 状态 3；关闭部分选项后槽位/位置/图标自动调整）
- 轮盘打开时**右键按住拖拽**可重排槽位
...
| 设置 | 说明 |
|---|---|
| 启用：原版选项 | **类总开关**：控制原版轮盘 7 项（要弹药/注意/帝皇/敌人/治疗/位置/谢谢） |
| 原版选项（折叠组） | 7 个独立子开关，逐项控制 |
| 启用：增强选项 | **类总开关**：控制 FTE 新增 3 项（帮助/是/否） |
| 增强选项（折叠组） | 3 个独立子开关，逐项控制 |
| 启用：报告高压 | 报告高压条目的开关 |
| 启用：状态输出 | **类总开关**：控制状态 3 项（大招/手雷/子弹）是否出现在轮盘上 |
| 状态条目（折叠组） | 大招状态 / 手雷数量 / 子弹余量，独立子开关 |
| 输出目标 | 仅本地 / 队伍频道（经 Vivox 发送，队友可见） |
| 发送冷却（秒） | 同类型消息最短发送间隔，0 = 不限 |
| 大招显示格式 | 自动 / 充能层数 / 冷却百分比 / 冷却秒数 |
| 显示备弹 | 子弹余量是否附带备弹池 |
| 轮盘行为 | 启用右键拖拽重排（关掉布局锁定防误触）、重复标记不喊"我的"（disable_dibs）、忽略求助（ignore_help） |
| 快捷键 | 14 个选项各自绑定按键（被禁用的选项对应快捷键同步失效） |
